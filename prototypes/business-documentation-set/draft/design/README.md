...
- **No persistence of anything derived.** The whole Cache Tree is reclaimable at any
  time, by the plugin or by the server. See
...
- **No `200` on a preview a client cannot display.** Nothing negotiates content
  type, size, or quality. One frame, one format, one quality.
