...
public sealed class CacheTreeSnapshot
{
    ...
    public static IReadOnlyDictionary<string, byte[]> Read(string root)
    {
    ...
    public static bool Matches(string root, IReadOnlyDictionary<string, byte[]> expected)
    {
        ArgumentNullException.ThrowIfNull(expected);
        try
        {
            IReadOnlyDictionary<string, byte[]> actual = Read(root);
            return actual.Count == expected.Count && expected.All(entry =>
